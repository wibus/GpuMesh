...
typedef float (*tetVolumeFct)(const vec3 vp[TET_VERTEX_COUNT], const Tet& tet);
extern __device__ tetVolumeFct tetVolumeImpl;


// Prism Volume evaluation function
typedef float (*priVolumeFct)(const vec3 vp[PRI_VERTEX_COUNT], const Pri& pri);
extern __device__ priVolumeFct priVolumeImpl;


// Hexahedron Volume evaluation function
typedef float (*hexVolumeFct)(const vec3 vp[HEX_VERTEX_COUNT], const Hex& hex);
